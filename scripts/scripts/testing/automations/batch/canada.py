...
class Canada:
    ...
    def read(self):
        df = pd.read_csv(
            self.source_url, usecols=["prname", "date", "numtested", "numtests"]
        )
    ...
    def pipeline_metric(
        self, df: pd.DataFrame, units: str, metric_field: str
    ) -> pd.DataFrame:
    ...
    def to_csv(self):
        ...
        output_path = os.path.join(
            "automated_sheets", f"{self.location} - people tested.csv"
        )
        df.pipe(self.pipeline_metric, "people tested", "numtested").to_csv(
            output_path, index=False
        )
        # Tests
        output_path = os.path.join(
            "automated_sheets", f"{self.location} - tests performed.csv"
        )
        df.pipe(self.pipeline_metric, "tests performed", "numtests").to_csv(
            output_path, index=False
        )
