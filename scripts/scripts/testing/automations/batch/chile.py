...
def main():
    ...
    tests = pd.read_csv(
        "https://github.com/MinCiencia/Datos-COVID19/raw/master/output/producto17/PCREstablecimiento_std.csv"
    )
    tests = tests[tests["Establecimiento"] == "Total realizados"]
    tests = tests[["fecha", "Numero de PCR"]].rename(
        columns={"fecha": "Date", "Numero de PCR": "Cumulative total"}
    )
    tests = tests.dropna()

    pr = pd.read_csv(
        "https://github.com/MinCiencia/Datos-COVID19/raw/master/output/producto55/Positividad_nacional.csv"
    )
    pr = pr.dropna()
