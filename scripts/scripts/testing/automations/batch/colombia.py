...
def main():
    ...
    ag["Positivas"] = (
        ag["Positivas"].str.replace(",", "").replace("", "0").astype(float)
    )
    ag["Total Px Ag"] = (
        ag["Total Px Ag"].str.replace(",", "").replace("", "0").astype(float)
    )
    ...
    df["Positive total"] = (
        df["positivas_acumuladas"].fillna(0).add(df["Positivas"].fillna(0))
    )
    ...
    df["Positive rate"] = (
        ...
        .div(
            (df["Cumulative total"] - df["Cumulative total"].shift(1)).rolling(7).mean()
        )
        .round(3)
    )
    ...
    df = df.drop(
        columns=[
            "positivas_acumuladas",
            "negativas_acumuladas",
            "Positivas",
            "Total Px Ag",
            "Positive total",
        ]
