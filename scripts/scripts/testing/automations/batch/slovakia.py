...
def main():
    ...
    df["Daily change in cumulative total"] = df["Dennych.PCR.testov"].fillna(0) + df[
        "AgTests"
    ].fillna(0)
    df["positive"] = df["Dennych.PCR.prirastkov"].fillna(0) + df["AgPosit"].fillna(0)
    df["Positive rate"] = (
        df.positive.rolling(7).mean()
        / df["Daily change in cumulative total"].rolling(7).mean()
    ).round(3)

    df = df[["Datum", "Daily change in cumulative total", "Positive rate"]].rename(
        columns={"Datum": "Date"}
    )
