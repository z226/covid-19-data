...
sample_official_data = [
    (
        ...
        {
            SERIES_TYPE: 75,
            "source": "https://fcsa.gov.ae/en-us/Pages/Covid19/UAE-Covid-19-Updates.aspx",
        },
    ),
    (
        "2020-02-08",
...
def get_data() -> pd.DataFrame:
    ...
    df = df.assign(**{DATE_COL: df[DATE_COL].apply(_ts_to_dt)})
    df[DATE_COL] = df[DATE_COL].dt.strftime("%Y-%m-%d")
    df.rename(
        columns={
            "TESTS": "Daily change in cumulative total",
