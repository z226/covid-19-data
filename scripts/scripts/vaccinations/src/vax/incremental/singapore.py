...
class Singapore:
    ...
    def _parse_data(self, soup: BeautifulSoup) -> pd.Series:
        data = pd.Series({
            ...
            "total_vaccinations": self._parse_metric(soup, "Total Doses Administered"),
            "people_vaccinated": self._parse_metric(soup, "Received at least First Dose"),
            "people_fully_vaccinated": self._parse_metric(soup, "Completed Full Vaccination Regimen"),
        })
        return data
